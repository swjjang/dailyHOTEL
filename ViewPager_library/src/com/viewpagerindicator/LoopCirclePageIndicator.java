...
public class LoopCirclePageIndicator extends View implements PageIndicator {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int count = 0;
        if (mViewPager.getAdapter() instanceof Loopable) {
			count = ((Loopable)mViewPager.getAdapter()).getRealCount();
		} else {
        ...
        if (mCurrentPage >= count) {
        	if (mViewPager.getAdapter() instanceof Loopable) {
        		mCurrentPage = ((Loopable)mViewPager.getAdapter()).getRealCurPos();
                invalidate();
        	} else {
    ...
    public void setCurrentItem(int item) {
        ...
        mCurrentPage = item % ((Loopable)mViewPager.getAdapter()).getRealCount();
        invalidate();
    }
    ...
    public void onPageSelected(int position) {
        ...
        if (mSnap || mScrollState == ViewPager.SCROLL_STATE_IDLE) {
            mSnapPage = position % ((Loopable)mViewPager.getAdapter()).getRealCount();
            invalidate();
        }
