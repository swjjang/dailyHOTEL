...
public class ImagesListActivity extends BaseActivity implements Constants
{
    ...
    public static Intent newInstance(Context context, PlaceType placeType, String title, List<ImageInformation> arrayList, int position)
    {
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        } else
        {
            try
            {
                mPlaceType = PlaceType.valueOf(intent.getStringExtra(INTENT_EXTRA_DATA_PLACETYPE));
            } catch (Exception e)
            {
    ...
    private void initToolbar(String title)
    {
        ...
        closeImageView.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                ...
                {
                    case HOTEL:
                        AnalyticsManager.getInstance(ImagesListActivity.this).recordEvent(AnalyticsManager.Category.HOTEL_BOOKINGS,//
                            AnalyticsManager.Action.HOTEL_IMAGE_CLOSED, AnalyticsManager.Label.CLOSE_, null);
                        break;

                    case FNB:
                        AnalyticsManager.getInstance(ImagesListActivity.this).recordEvent(AnalyticsManager.Category.GOURMET_BOOKINGS,//
                ...
                switch (action)
                {
                    ...
                    case MotionEvent.ACTION_UP:
                    {
                        ...
                        if (mIsMoved == true || mListView.getTranslationY() != 0.0f)
                        {
                            ...
                            if (yVelocity > 5.0f || Math.abs(mListView.getTranslationY()) > (ScreenUtils.getScreenHeight(ImagesListActivity.this) / 4))
                            {
                                switch (mPlaceType)
                                {
                                    case HOTEL:
    ...
    void scrollListEffect(float y)
    {
        ...
        mAlphaView.setAlpha(1.0f - Math.abs(y * 1.5f) / ScreenUtils.getScreenHeight(ImagesListActivity.this));
