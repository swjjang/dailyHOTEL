...
public final class AppboyGcmReceiver extends BroadcastReceiver
{
    ...
    boolean handleRegistrationIntent(Context context, Intent intent)
    {
        ...
        if (error != null)
        {
            ...
            {
                ExLog.e(TAG + "Unable to connect to the GCM registration server. Try again later.");
            } else if ("ACCOUNT_MISSING".equals(error))
            {
                ExLog.e(TAG + "No Google account found on the phone. For pre-3.0 devices, a Google account is required on the device.");
            } else if ("AUTHENTICATION_FAILED".equals(error))
            {
                ExLog.e(TAG + "Unable to authenticate Google account. For Android versions <4.0.4, a valid Google Play account " + "is required for Google Cloud Messaging to function. This phone will be unable to receive Google Cloud " + "Messages until the user logs in with a valid Google Play account or upgrades the operating system on this device.");
            } else if ("INVALID_SENDER".equals(error))
            {
                ExLog.e(TAG + "One or multiple of the sender IDs provided are invalid.");
            } else if ("PHONE_REGISTRATION_ERROR".equals(error))
            {
                ExLog.e(TAG + "Device does not support GCM.");
            } else if ("INVALID_PARAMETERS".equals(error))
            {
                ExLog.e(TAG + "The request sent by the device does not contain the expected parameters. This phone does not " + "currently support GCM.");
            } else
            {
    ...
    boolean handleAppboyGcmMessage(Context context, Intent intent)
    {
        ...
        if (GCM_DELETED_MESSAGES_KEY.equals(messageType))
        {
            ...
            if (totalDeleted == -1)
            {
                ExLog.e(TAG + String.format("Unable to parse GCM message. Intent: %s", intent.toString()));
            } else
            {
        ...
        } else
        {
            ...
            if (AppboyNotificationUtils.isNotificationMessage(intent))
            {
                ...
                {
                    ExLog.e(TAG + "Failed to create notification." + e.toString());
                    return false;
                }
