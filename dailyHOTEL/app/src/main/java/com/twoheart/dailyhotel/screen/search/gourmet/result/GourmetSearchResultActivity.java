    ...
    protected void onResearchActivityResult(int resultCode, Intent data)
    {
        if (resultCode == Activity.RESULT_OK && data != null)
        {
            try
            {
                ...
                if (gourmetSuggest.isLocationSuggestType() == true)
                {
    ...
    protected void changeViewType()
    {
        ...
        switch (mViewType)
        {
            ...
            case MAP:
            {
                ...
                AnalyticsManager.getInstance(GourmetSearchResultActivity.this).recordEvent(AnalyticsManager.Category.NAVIGATION_, AnalyticsManager.Action.CHANGE_VIEW, AnalyticsManager.Label._GOURMET_LIST, null);
                break;
            }
        }
    ...
    GourmetSearchResultLayout.OnEventListener mOnEventListener = new GourmetSearchResultLayout.OnEventListener()
    {
        ...
        public void onItemSelectedSpinner(double radius)
        {
            ...
            try
            {
                ...
                String label = suggest != null && suggest.isLocationSuggestType() == true ? mAddress : suggest.getText1();
