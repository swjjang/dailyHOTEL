...
public class FeedbackMailActivity extends BaseActivity implements Constants, OnClickListener, View.OnFocusChangeListener
{
    ...
    private DailyHotelJsonResponseListener mUserChangePwJsonResponseListener = new DailyHotelJsonResponseListener()
    {
        ...
        {
            FeedbackMailActivity.this.onErrorResponse(volleyError);
        }
    };
        ...
        public void onResponse(String url, Map<String, String> params, JSONObject response)
        {
            try
            {
                ...
                if ("true".equalsIgnoreCase(result) == true)
                {
                    if (Util.isTextEmpty(mEmail) == true)
                    {
                        DailyToast.showToast(FeedbackMailActivity.this, R.string.toast_msg_please_input_email, Toast.LENGTH_SHORT);
                    } else
                    {
                        DailyNetworkAPI.getInstance(FeedbackMailActivity.this).requestUserChangePassword(mNetworkTag, mEmail, mUserChangePwJsonResponseListener);
                    }
                } else
                {
                    unLockUI();
