...
public class StayMainActivity extends PlaceMainActivity
{
        ...
        switch (resultCode)
        {
            ...
            case com.daily.base.BaseActivity.RESULT_CODE_START_CALENDAR:
                if (data != null && data.hasExtra(StayAreaListActivity.INTENT_EXTRA_DATA_REGION) == true)
                {
                    ...
                    mStayCuration.setRegion(region);
                    ...
                    mPlaceMainLayout.setCategoryTabLayout(getSupportFragmentManager(), region.getArea().getCategoryList(), //
        ...
        public void onDateTime(TodayDateTime todayDateTime)
        {
            ...
            try
            {
                ...
                } else
                {
                    ...
                    addCompositeDisposable(mStayRemoteImpl.getRegionList(DailyCategoryType.STAY_ALL).observeOn(AndroidSchedulers.mainThread()).subscribe(new Consumer<List<StayAreaGroup>>()
                    {
                        @Override
                        public void accept(List<StayAreaGroup> areaGroupList) throws Exception
                        {
                            if (mDailyDeepLink != null && mDailyDeepLink.isValidateLink() == true//
                                && processDeepLinkByRegionList(StayMainActivity.this, areaGroupList, mTodayDateTime, mDailyDeepLink) == true)
                            {
                            ...
                            } else
                            {
                                ...
                                if (region == null)
                                {
                                    region = searchRegion(areaGroupList, getDistrictNTownNameByCategory(DailyCategoryType.STAY_ALL));
                                }
            ...
            switch (placeViewItem.mType)
            {
                case PlaceViewItem.TYPE_ENTRY:
                {
                    ...
                    if (Util.isUsedMultiTransition() == true)
                    {
                        ...
                        {
                            optionsCompat = ActivityOptionsCompat.makeSceneTransitionAnimation(StayMainActivity.this, ((DailyStayMapCardView) view).getOptionsCompat());
    ...
    boolean moveDeepLinkStayList(List<StayAreaGroup> stayDistrictList, TodayDateTime todayDateTime, DailyDeepLink dailyDeepLink)
    {
        if (dailyDeepLink == null)
        {
            return false;
