	...
	public static void setSessionCookie() {
		...
		if (cookies != null) {
			for (int i=0; i<cookies.size(); i++) {
				...
				if (cookie.getName().equals(KEY_DAILYHOTEL_COOKIE)) {
					sCookie = cookie;
					
					StringBuilder cookieString = new StringBuilder();
					cookieString.append(sCookie.getName()).
					append("=").append(sCookie.getValue());
					
					sCookieManager.setCookie(URL_DAILYHOTEL_SERVER, cookieString.toString());
